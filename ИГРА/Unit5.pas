...
function exit(pole:tpole):boolean;
var a:integer;
begin
  a:=0;
  if (i2+1<=n-1) and (pole[i2+1,j2]=-12) then
  begin
    napr:=1;
    a:=1;
  end;
  if (i2-1>0) and (pole[i2-1,j2]=-12) then   
  begin
    napr:=3;
    a:=1;
  end;
  if (j2+1<=n-1) and (pole[i2,j2+1]=-12) then    
  begin
    napr:=4;
    a:=1;
  end;
  if (j2-1>0) and (pole[i2,j2-1]=-12) then     
  begin
    napr:=2;
    a:=1;
  end;
...
procedure prov2;      //Определение стороны в которую надо идти стрелки
...
begin
  ...
  for i1:=1 to n-1 do
    for j1:=1 to n-1 do
      if p[i1,j1]=-12 then
      begin
        if (i1+1<=n-1) and (p[i1+1,j1]<min) and (p[i1+1,j1]>0) then
        begin
          min:=p[i1+1,j1];
          i0:=i1+1;
          j0:=j1;
        end;
        if (i1-1>0) and (p[i1-1,j1]<min) and (p[i1-1,j1]>0) then
        begin
          min:=p[i1-1,j1];
          i0:=i1-1;
          j0:=j1;
        end;
        if (j1+1<=n-1) and (p[i1,j1+1]<min) and (p[i1,j1+1]>0) then
        begin
          min:=p[i1,j1+1];
          i0:=i1;
          j0:=j1+1;
        end;
        if (j1-1>0) and (p[i1,j1-1]<min) and (p[i1,j1-1]>0) then
        begin
          min:=p[i1,j1-1];
          i0:=i1;
          j0:=j1-1;
